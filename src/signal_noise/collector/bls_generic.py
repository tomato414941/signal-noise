...
def _make_bls_collector(
    ...
    class _Collector(BaseCollector):
        ...
        def fetch(self) -> pd.DataFrame:
            ...
            # BLS allows max 20 years per request
            for y_start in range(start_year, end_year + 1, 20):
                y_end = min(y_start + 19, end_year)
                payload = {
                    "seriesid": [series_id],
                ...
                series_data = body.get("Results", {}).get("series", [])
                if not series_data:
                    continue
                for item in series_data[0].get("data", []):
                    ...
                    if not year or not val:
                        continue
                    # M01-M12 = monthly, Q01-Q05 = quarterly, S01-S02 = semiannual
